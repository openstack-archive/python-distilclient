...
from distilclient.v2 import client as distil_client


def _get_client(obj, parsed_args):
    obj.log.debug("take_action(%s)" % parsed_args)
    distilclient = distil_client.Client(service_type="ratingv2",
                                        session=obj.app.client_manager.session)
...
class ListProducts(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        data = client.products.list(**kwargs)
        products = []
